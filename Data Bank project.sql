...
select r.region_name region, count(c.node_id)
from regions r
join customer_nodes c on c.region_id=r.region_id
group by region

--3. How many customers are allocated to each region?
select region_name region, count(distinct customer_id)
from regions r
join customer_nodes c on c.region_id=r.region_id
group by region

--4.****How many days on average are customers reallocated to a different node
SELECT AVG(end_date - start_date) AS average
FROM customer_nodes
...
SELECT region_name,
PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY r.region_id) as Median,
PERCENTILE_CONT(0.8) WITHIN GROUP(ORDER BY r.region_id) as percentile_80,
PERCENTILE_CONT(0.95) WITHIN GROUP(ORDER BY r.region_id) as percentile_95
FROM regions r
join customer_nodes c on c.region_id=r.region_id
...
select count(txn_type), avg(txn_amount)
from customer_transactions
...
select to_char('Month', txn_type), customer_id
case when txn_type 
